...
from jax_filters import apply_filtering_fixed_nonlinear_H, kalman_filter_process, ensrf_steps_H
from jax_models import visualize_observations, Lorenz63, generate_true_states_H, generate_localization_matrix
...
num_train_steps = 700  # Number of training time steps
...
nonlinear_observation = Partial(H)
nonlinear_observation_jacobian = Partial(H_jac)
...
observations, true_states = generate_true_states_H(
    key, num_steps, n, initial_state, nonlinear_observation, Q, R, model_step, observation_interval
)
...
schedule = optax.cosine_decay_schedule(init_value=initial_lr, decay_steps=num_train_steps, alpha=min_lr_factor)
...
m_preds, C_preds, m_updates, C_updates, Ks = kalman_filter_process_H(
    state_transition_function, jac_func, m0, C0, observations, nonlinear_observation, nonlinear_observation_jacobian, Q, R
)
...
ensemble_preds, C_preds, ensembles, covariances = ensrf_steps_H(
    state_transition_function=state_transition_function,
    ensemble_init=ensemble_init,
    num_steps=num_steps,
    observations=observations,
    observation_interval=1,
    observation_function=nonlinear_observation,  # Nonlinear H(x)
    observation_jacobian=nonlinear_observation_jacobian,  # Jacobian of H(x)
    Q=Q,
    R=R,
